...
async function ProductPage({ params: { id } }: Props) {
  ...
  // for vercel
  const reponse = await fetch(
    `https://${process.env.VERCEL_URL}/api/shopping/product/${id}`
  );
  const productData = (await reponse.json()) as ProductData;
  ...
  return (
    <div className="p-12 pt-0">
      ...
      {productData.content.reviews && (
        <div className="flex space-x-1">
          {[
            ...
            <StarIcon key={i} className="h-5 w-5 text-yellow-500"></StarIcon>
          ))}
          {/* showing remaining stars from review out of 5 */}
          {[
            ...Array.from({
              length: 5 - Math.round(productData.content.reviews.rating),
            }),
          ].map((_, i) => (
            <StarIcon key={i} className="h-5 w-5 text-gray-500"></StarIcon>
          ))}
        </div>
      )}
              ...
              <>
                ...
                <div className="flex space-x-4">
                  <p className="text-sm text-gray-600">
                    ...
                    {productData.content.pricing.online[0].price_tax} tax)
                  </p>
      ...
      {productData.content.specification && (
        <section>
          <hr className="my-10" />
          <h3 className="font-bold text-2xl">Specifications</h3>
          <div className="flex space-x-5 flex-wrap">
            {productData.content.specification.map((spec) => (
